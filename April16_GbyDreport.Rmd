...
date: "December 2023, edited 4/16/24"
...
d1 %>%
  group_by(course, covid) %>%
  summarize( DFW = sum(ABCDFW == "DFW")/n(), students = n(), DFW_students=sum(ABCDFW == "DFW"), .groups = "drop" ) %>%
  group_by(course) %>%
  mutate(DFW_min = min(DFW), DFW_max = max(DFW) ) %>%
  mutate(students_in_gap = students[which.max(DFW)]*(max(DFW)-min(DFW)), diversity = students/sum(students), total_enrollment = 
           sum(students) ) -> b4maxCheck
...
if (b4maxCheck$DFW_max[1] > 0.5) {
  ...
  sorted_d1 %>%
    pull(covid) -> cat_sorted
    
  sorted_d1 %>%
  mutate(cat_factors = factor(covid, levels = cat_sorted)) %>% 
  ...
  geom_point_interactive(aes(size = students, tooltip = paste0(
    course,"\r",
    covid," (",percent(diversity,accuracy = 0.1)," of students)\r",
    DFW_students," of ",students," enrollments\r",
    percent(DFW, accuracy = 0.1)," DFW" )) , shape = 21) +
  scale_x_continuous(labels = scales::percent, limits = c(0,upperLimit)) +
  guides(fill = guide_legend(override.aes = list(size = 10))) +
  labs(fill = "Covid", y = "Course") +
  ggtitle(paste("Covid", "DFW Rates")) -> plot
...
if (b4maxCheck$DFW_max[1] > 0.5) {
  ...
  sorted_d1 %>%
    pull(firstgen) -> cat_sorted
    
  sorted_d1 %>%
  mutate(cat_factors = factor(firstgen, levels = cat_sorted)) %>% 
  ...
  ggtitle(paste("Generation", "DFW Rates")) -> plot
girafe(ggobj=plot, width=7)
...
if (b4maxCheck$DFW_max[1] > 0.5) {
  ...
  sorted_d1 %>%
    pull(gender) -> cat_sorted
    
  sorted_d1 %>%
  mutate(cat_factors = factor(gender, levels = cat_sorted)) %>% 
  ...
  geom_point_interactive(aes(size = students, tooltip = paste0(
    course,"\r",
    gender," (",percent(diversity,accuracy = 0.1)," of students)\r",
    DFW_students," of ",students," enrollments\r",
    percent(DFW, accuracy = 0.1)," DFW" )) , shape = 21) +
  scale_x_continuous(labels = scales::percent, limits = c(0,upperLimit)) +
  guides(fill = guide_legend(override.aes = list(size = 10))) +
  labs(fill = "Gender", y = "Course") +
  ggtitle(paste("Gender", "DFW Rates")) -> plot
girafe(ggobj=plot, width=7)
...
if (b4maxCheck$DFW_max[1] > 0.5) {
  ...
  sorted_d1 %>%
  mutate(cat_factors = factor(race_ethnicity, levels = cat_sorted)) %>% 
  ...
  geom_point_interactive(aes(size = students, tooltip = paste0(
    course,"\r",
    race_ethnicity," (",percent(diversity,accuracy = 0.1)," of students)\r",
    DFW_students," of ",students," enrollments\r",
    percent(DFW, accuracy = 0.1)," DFW" )) , shape = 21) +
  scale_x_continuous(labels = scales::percent, limits = c(0,upperLimit)) +
  guides(fill = guide_legend(override.aes = list(size = 10))) +
  labs(fill = "Race/Ethnicity", y = "Course") +
  ggtitle(paste("Race/Ethnicity", "DFW Rates")) -> plot
girafe(ggobj=plot, width=7)
...
# Copy of Visualizations without mouseover
for (i in keptCols) { 
d1 %>%
  group_by(course, !!sym(i)) %>%
