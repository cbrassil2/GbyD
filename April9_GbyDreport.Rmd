...
date: "December 2023, edited 4/9/24"
...
if (length(droppedCols) != 0) {
  cat("Aggregated Columns:",droppedCols)
}
```
...
for (i in keptCols) {
  ...
  mutate(Gap = max(across(!matches("course"))) - min(across(!matches("course"))), demCat = " ") %>%
  relocate(Gap, .after = "course") %>%
...
# Pair with GitHub
# library(usethis)
# use_git_config(user.name = "Tim Gabriel", user.email = "tgabriel@nebrwesleyan.edu")
```
...
if (length(droppedColsInt) != 0) {
  cat("Aggregated Columns:",droppedColsInt)
}
```
...
for (i in keptColsInt) {
  
d1 %>%
    group_by(course,!!sym(i[1]),!!sym(i[2])) %>%
    summarize(students=n(), .groups = "drop" ) %>%
    pivot_wider(names_from = c(!!i[1],!!i[2]), values_from=students) %>%
    mutate(Gap = max(across(!matches("course"))) - min(across(!matches("course"))), demCat = " ") %>%
    relocate(Gap, .after = "course") %>%
    relocate(demCat, .after = "Gap") %>%
