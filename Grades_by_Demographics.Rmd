...
```{r Data and Options, include=FALSE, message = FALSE}
# Have data file in the same folder as this R Markdown file.

ds <- read_csv("data_grade_demographics.csv")

### data file structure ###
# By default, the code looks for data_grade_demographics.csv file in the same folder in which you have this R file. The default file contains synthetic data which matches real general demographic patterns, but which does represent any individual section or student. 
# You can replace that with your own institutional data. Each row is an individual student for an individual course in a single term. All data for all courses and all terms is listed in a single table, with columns for course and term. A student may appear in the data set multiple times if they repeated a course or if multiple courses are represented in the data. 
#
...
#course_prefix can be used to create a report for a subset of courses in the data file. In the YAML header, which are the lines at the very top of the this file change c("all") to a single course, like c("Calc1") or a list, for example c("Calc","Chem") to include all courses that begin with the listed prefixes. The Batch_Script_for_Grades_by_Demographics.R can be used along with Batch_Course_Codes.csv to create separate reports for subsets of course. 

area_is_total <- TRUE #Set to TRUE to scale the area of points to the total number of student, i.e. the denominator in DFW percent. This may help visualize the distribution of students across categories. Set to FALSE to scale the area of points to the number of DFW students, i.e. the numerator in DFW percent. This may help visualize categories in which you have lots of DFW students. However, be aware that setting area_is_total to FALSE will result in points disappearing if all students for a category succeeded. Note, if showing GPA, the area for GPA graphs will always be total number of students.
...
hide_NA <- TRUE #Set to TRUE to hide NAs. Set to FALSE to show NA's in fields.

show_GPA_excluding_DF <- FALSE #Set to FALSE to hide GPA exclude DF figures and tables and show DFW not 1-DFW. Set to TRUE to show GPA excluding DF and switch DFW to Success = 1-DFW so that direction of figures is aligned. 
...
# List of equivalent column names here. Additional values can be added to make the code more robust to variation in the way users format data. For example, capitalization options and common word substitution have been included.
courseColumns <- c("course","Course","class","Class")
gradeColumns <- c("grade","Grade","Letter_Grade","letter_grade", "Letter Grade", "letter grade")
termColumns <- c("term","Term","semester","Semester","year_and_semester","Year_and_Semester","academic_year","Academic_year")
...
#  filter( course_prefix == "all" | course %in% which_courses) %>% #filter courses if which_courses is set to anything besides "all"
  filter( params$course_prefix == "all" | str_starts(course,paste(params$course_prefix,collapse = "|")) ) %>% #filter courses if course_prefix is set to anything besides "all"
...
ds %>%
  ...
  mutate(ABCDFW = case_when(
    ...
    TRUE ~ NA)) %>%
  filter(!is.na(ABCDFW)) -> dt #remove any lines with an NA for ABCDFW
...
`r if(area_is_total) "In the figures, the area of the circles are proportional to the total number of student enrollments in the category." else"In the figures, the area of the circles are proportional to the number of DFW students in the category."` Mouse over a circle for specific details on the number of student enrollments, the percentage of enrollments that represents, and details on grades.
...
if (droppedCols != "") cat("\nThe following are not shown because they did not meet minimum threshold of student enrollments for all categories:\n\n",droppedCols,'\n', sep="")
...
dt %>%
  ...
  {if (show_GPA_excluding_DF) mutate(.,to_sum = ABCDFW != "DFW") else mutate(.,to_sum = ABCDFW == "DFW") } %>% #Success or DFW
  summarize(DFW_rate = if_else( censor_by_removal & n() < threshold,NA,sum(to_sum)/n()), DFW = if_else( censor_by_removal & n() < threshold, "Below Threshold", paste0(percent(sum(to_sum)/n(),accuracy=0.1),"<br><span style='white-space: nowrap;'>(",sum(to_sum)," of ",n(),")</span>")), .groups = "drop" ) %>%
  group_by(course) %>%
  mutate(Gap = max(DFW_rate, na.rm=TRUE) - min(DFW_rate, na.rm=TRUE)) %>%
  mutate(Gap = if_else(is.infinite(Gap) | Gap == 0,NA,Gap)) %>% #replace infinite values or zero values (typically becuase no only a single value) with NA, which will be blank  
  select(-DFW_rate) %>%
...
dt %>%
  ...
  mutate(Gap = if_else(is.infinite(Gap) | Gap == 0,NA,Gap)) %>% #replace infinite values or zero values (typically because no only a single value) with NA, which will be blank
  relocate(Gap, .after = "course") %>%
  mutate(demCat = i) %>% 
  group_by(demCat) %>%
  gt(rowname_col="course") %>% # Table creation starts here
...
dt %>%
  {if(censor_by_removal == FALSE) right_join(.,courseList, by = c("course")) else .} %>% #only keep the courses that met thresholds for this demographic
  filter(if_else( hide_NA & ( is.na(!!sym(i)) | !!sym(i) %in% c("","NA","NaN") ),FALSE,TRUE) ) %>% #filter out NA values or equivalent
  mutate(course = fct_rev(course)) %>% #reverse courses so that they match the order of the table
  group_by(course, !!sym(i)) %>%
  filter(if_else( censor_by_removal & n() < threshold,FALSE,TRUE)) %>% #filter out censor_by_removal categories
  mutate(GPA_exclude_DFW = if_else(ABCDFW=="ABC",grade_point,NaN)) %>%
...
if (show_GPA_excluding_DF == FALSE){
  lowerLimit <- 0;
  # Sets scale of x-axis to 50% or highest Max + 10%
  if (max(b4maxCheck$DFW_max, na.rm=TRUE) > 0.5) {
    upperLimit <- max(b4maxCheck$DFW_max, na.rm=TRUE) + 0.1} else {
      upperLimit <- 0.5};
  unit_text = "DFW";
  title_text = "DFW Rates"
} else {
  # Sets scale of x-axis to 50% or lowest Min - 10%
  if (min(1-b4maxCheck$DFW_max, na.rm=TRUE) < 0.5) {
    
    lowerLimit <- min(1-b4maxCheck$DFW_max, na.rm=TRUE) - 0.1} else {
      lowerLimit <- 0.5};
  upperLimit <- 1;
  unit_text = "Success";
  title_text = "Success Rates (1-DFW)"
}

#DFW or Success
b4maxCheck %>%
  rowwise() %>% #needed to force the calculation of size_variable at the row_level
  mutate(size_variable = if_else(area_is_total==TRUE,students,DFW_students)) %>%
  ungroup() %>%
  mutate(cat_factors = fct_reorder(!!sym(i), DFW, .desc=TRUE)) %>% #orders legend on DFW rates
  group_by(course) %>%
  arrange(desc(size_variable)) %>% #orders circles so biggest in the back for each course
  {if (show_GPA_excluding_DF) mutate(.,DFW = 1-DFW, DFW_min_temp = DFW_min, DFW_min = 1- DFW_max, DFW_max = 1 - DFW_min_temp ) else .} %>% #if success, swap data
  ...
  geom_segment_interactive(aes(x = DFW_min, xend = DFW_max, yend=course, tooltip = paste(
    ...
    ), data_id = course), colour = "gray", linewidth = 15) +
  scale_size_area(max_size= 15, breaks = NULL, guide = "none") +
  geom_point_interactive(aes(size = size_variable, tooltip = paste0(
    course,"\r",
    !!sym(i)," (",percent(diversity,accuracy = 0.1)," of enrollments)\r",
    percent(DFW, accuracy = 0.1)," ",unit_text," (",DFW_students," of ",students," enrollments)"),
    data_id = cat_factors:course) , shape = 21) +
  scale_x_continuous(labels = scales::percent, limits = c(lowerLimit,upperLimit)) +
  guides(fill = guide_legend_interactive(override.aes = list(size = 10), title= i)) +
  labs(fill = sym(i), y = "", x = unit_text, title = title_text) -> plot


# Sets scale of x-axis to 3.0 or lowest GPA - 0.1
if (min(b4maxCheck$GPA_exclude_DFW_min, na.rm=TRUE) < 3.0) {
  lowerLimit <- min(b4maxCheck$GPA_exclude_DFW, na.rm=TRUE) - 0.1} else {
    lowerLimit <- 3.0}

# Sets scale of x-axis to 3.5 or highest GPA + 0.1
if (max(b4maxCheck$GPA_exclude_DFW, na.rm=TRUE) > 3.5) {
  upperLimit <- max(b4maxCheck$GPA_exclude_DFW, na.rm=TRUE) + 0.1} else {
    upperLimit <- 3.5}
...
b4maxCheck %>%
  ...
  geom_point_interactive(aes(size = students, tooltip = paste0(
    ...
    number(GPA_exclude_DFW, accuracy = 0.01)," GPA excluding DF"), data_id = cat_factors:course) , shape = 21) +
...
print(htmltools::tagList(girafe(ggobj = plot, options=list(opts_hover(girafe_css(css="", point = "stroke-width:2", line = "stroke:DarkGray")) ) ))) # Use htmltools::tagList() to generate multiple interactive plots # results = 'asis' must go in {}

print(htmltools::tagList(table))

if(show_GPA_excluding_DF) cat('\n###', i , "- GPA excluding DF",  '\n')

if(show_GPA_excluding_DF) print(htmltools::tagList(girafe(ggobj = plot_GPA, options=list(opts_hover(girafe_css(css="", point = "stroke-width:2", line = "stroke:DarkGray"))))))
...
-   Mouse over the circles to consider the percentage of student enrollments in each demographic combination. Is there a combination of demographics that are disproportionately represented in a course? Does that surprise you? Alternatively, does it surprise you that not all students are in that combination of demographics?
...
dt %>%
  ...
  summarize(DFW_rate = if_else( censor_by_removal & n() < threshold,NA,sum(to_sum)/n()), DFW = if_else( censor_by_removal & n() < threshold, "Below Threshold", paste0(percent(sum(to_sum)/n(),accuracy=0.1),"<br><span style='white-space: nowrap;'>(",sum(to_sum)," of ",n(),")</span>")), .groups = "drop" ) %>%
    group_by(course) %>%
    mutate(Gap = max(DFW_rate, na.rm=TRUE) - min(DFW_rate, na.rm=TRUE)) %>% # Calculates the gap percentage
    mutate(Gap = if_else(is.infinite(Gap) | Gap == 0,NA,Gap)) %>% #replace infinite values or zero values (typically because only a single value) with NA, which will be blank
    select(-DFW_rate) %>%
...
b4maxCheck %>%
  ...
  mutate(size_variable = if_else(area_is_total==TRUE,students,DFW_students)) %>%
  ungroup() %>%
  mutate(intersection = fct_reorder(intersection, DFW, .desc=TRUE)) %>% #orders legend on DFW rates
  group_by(course) %>%
  arrange(desc(size_variable)) %>% #orders circles so biggest in the back for each course
  {if (show_GPA_excluding_DF) mutate(.,DFW = 1-DFW, DFW_min_temp = DFW_min, DFW_min = 1- DFW_max, DFW_max = 1 - DFW_min_temp ) else .} %>% #if success, swap data
  ggplot(aes(x = DFW, y = course, size = size_variable, fill = intersection)) + #Plotting code starts here
  theme_classic() +
  theme(axis.text = element_text_interactive(size = 12) ) +
  geom_segment_interactive(aes(x = DFW_min, xend = DFW_max, yend=course, tooltip = paste(
    course,"\r",
  ...
  geom_point_interactive(aes(size = size_variable, tooltip = paste0(
    course,"\r",
    intersection," (",percent(diversity,accuracy = 0.1)," of enrollments))\r",
    percent(DFW, accuracy = 0.1)," ",unit_text," (",DFW_students," of ",students," enrollments)"),
    data_id = intersection:course) , shape = 21) +
  scale_x_continuous(labels = scales::percent, limits = c(lowerLimit,upperLimit)) +
  guides(fill = guide_legend(override.aes = list(size = 5), title="")) +
    labs(y = "", x = unit_text, title = title_text)-> plot
  
  #Creates title in Markdown format
  cat('\n##', i , '\n')
  #prints interactive visualization
  print(htmltools::tagList(girafe(ggobj = plot, options=list(opts_hover(girafe_css(css="", point = "stroke-width:2", line = "stroke:DarkGray"))) )))
  #prints table
  print(htmltools::tagList(table))
}
#}
...
```{r Across Time, results='asis', echo=FALSE, include=TRUE, eval = TRUE, warning = FALSE}
#initialize
metThresholds  <- tibble(
  course = factor(),
  term = factor(),
...
metThresholds %>%
  group_by(course,demograph) %>%
  filter( sum(met) > 1) %>% #remove if there are not at least 2 time periods to plot
...
for(j in levels(combosToPlot$course)) {
  ...
  for (i in levels((combosToPlot %>% filter(course == j) %>% mutate(demograph = factor(demograph)))$demograph)) { #only look at demograph levels that are in combosToPlot for that course, i.e. that haven't been removed.
...
b4maxCheck %>%
  ...
  mutate(cat_factors = fct_reorder(!!sym(i), DFW, .desc=TRUE)) %>% #orders legend on DFW rates
  group_by(term) %>%
  arrange(desc(size_variable)) %>% #orders circles so biggest in the back for each term
  {if (show_GPA_excluding_DF) mutate(.,DFW = 1-DFW, DFW_min_temp = DFW_min, DFW_min = 1- DFW_max, DFW_max = 1 - DFW_min_temp ) else .} %>% #if success, swap data
  ...
  geom_segment_interactive(aes(y = DFW_min, yend = DFW_max, xend=term, tooltip = paste(
    ...
    "Students in Gap:",round(students_in_gap, digits = 1),"\r",
    "Total Enrollments:",total_enrollment,"\r"
    ), data_id = term), colour = "gray", linewidth = 15) +
    scale_size_area(max_size= 15, breaks = NULL, guide = "none") +
  geom_point_interactive(aes(size = size_variable, tooltip = paste0(
    term,"\r",
    !!sym(i)," (",percent(diversity,accuracy = 0.1)," of enrollments))\r",
    percent(DFW, accuracy = 0.1)," ",unit_text," (",DFW_students," of ",students," enrollments)"),
    data_id = cat_factors:term) , shape = 21) +
  scale_y_continuous(labels = scales::percent, limits = c(lowerLimit,upperLimit)) +
  guides(fill = guide_legend(override.aes = list(size = 10), title="")) + #include title above legend with title = i
  theme(axis.text.x=element_text(angle = 90, hjust = 0)) +
  xlab("Time") +
  labs(fill = sym(i),y = unit_text, title = title_text) -> plot
...
`r if(censor_by_removal == FALSE) "A demographic column must meet the minimum threshold for number of student enrollments in all categories in order to be displayed. If a column does not meet the minimum threshold, and you anticipate that it should, check your data for typos which could be inadvertently creating another category in that column."`
...
-   Adjust the minimum threshold for required for a demographic factor to be displayed.
...
Contributors to this code include Tim Gabriel and Chad E. Brassil at the University of Nebraska-Lincoln. Funding for this report was provided by HHMI as part of the IE3 program. This report was inspired by and uses code from the [SELC Equity Report](https://github.com/centerforbiophotonics/SEISMIC-equity-measures) which is part of the [SEISMIC Project](https://www.seismicproject.org/). The code for this report is available at https://github.com/cbrassil2/GbyD
